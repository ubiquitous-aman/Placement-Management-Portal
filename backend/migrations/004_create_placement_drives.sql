...
CREATE TABLE IF NOT EXISTS placement_drives (
  ...
  minimum_cgpa         NUMERIC(4, 2)  NOT NULL DEFAULT 0.00
                         CHECK (minimum_cgpa >= 0.00 AND minimum_cgpa <= 10.00),
  maximum_backlogs     INTEGER        NOT NULL DEFAULT 0
                         CHECK (maximum_backlogs >= 0),
...
CREATE INDEX IF NOT EXISTS idx_placement_drives_deadline ON placement_drives (application_deadline);

-- Index for eligibility filtering by graduation year
CREATE INDEX IF NOT EXISTS idx_placement_drives_grad_year ON placement_drives (graduation_year);

COMMENT ON TABLE  placement_drives                   IS 'Individual placement drives run by companies';
